...
public class MessengerManager {
    ...
    public void sendMessage(String recipientId, Message message, IFacebookListener listener) {
        try {
            JSONObject messageObject = new JSONObject("{\"recipient\":{},\"message\":{}}");
            messageObject.getJSONObject("recipient").put("id", recipientId);
            messageObject.put("message", new JSONObject(Globalizer.jsonMapper().writeValueAsString(message)));
            Response response = postRequest(MESSAGE_API + PAGE_ACCESS_TOKEN, messageObject.toString());
    ...
    public void sendJsonTemplate(String recipientId, String jsonFileURL, IFacebookListener listener) {        
        try {
            ...
            this.sendMessage(recipientId, new Message("Invalid Template File."), listener);
            LOG.error(ex);
        }
    }
}
