...
import com.sdm.core.mysql.model.type.PropertyType;
import com.sdm.core.mysql.model.type.Text;
...
public class AdminDAO extends MySQLDAO {
    ...
    private PropertyType typeConverter(String sql) {
        ...
        if (sql.contains("INT") || sql.contains("INTEGER")) {
            return new com.sdm.core.mysql.model.type.Integer(sql);
        }

        if (sql.startsWith("FLOAT") || sql.startsWith("DOUBLE") || sql.startsWith("DECIMAL")) {
            return new com.sdm.core.mysql.model.type.Float(sql);
        }

        if (sql.contains("DATE") || sql.contains("TIME") || sql.startsWith("YEAR")) {
            return new com.sdm.core.mysql.model.type.DateTime(sql);
        }

        if (sql.startsWith("ENUM")) {
            return new com.sdm.core.mysql.model.type.List(sql);
        }

        if (sql.equalsIgnoreCase("BIT(1)")) {
            return new com.sdm.core.mysql.model.type.Boolean();
        }
    ...
    public List<PropertyModel> descObject(String objectName) throws SQLException {
        ...
        try (ResultSet resultSet = executeQuery(query)) {
            ...
            while (resultSet.next()) {
                ...
                if (extra != null && model.getType() instanceof com.sdm.core.mysql.model.type.Integer) {
                    com.sdm.core.mysql.model.type.Integer currentType = (com.sdm.core.mysql.model.type.Integer) model.getType();
